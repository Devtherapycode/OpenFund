...
public class GoogleOAuthServiceTests
{
    private static (
        ...
        Mock<ILogger<GoogleOAuthService>> logger,
        GoogleOAuthService sut)
        BuildSut()
    {
        ...
        var logger = new Mock<ILogger<GoogleOAuthService>>();

        var sut = new GoogleOAuthService(
            userManager.Object,
            jwtProvider.Object,
            tokenStorage.Object,
            logger.Object);
    ...
    [Fact]
    public async Task HandleCallbackAsync_WhenUserCreationFails_ReturnsFailureResult()
    {
        var (userManager, jwtProvider, tokenStorage, _, sut) = BuildSut();

        var authResult = CreateAuthenticateResult("google123", "new@mail.com", "New User");

        userManager.Setup(um => um.FindByEmailAsync("new@mail.com"))
            .ReturnsAsync((User?)null);

        userManager.Setup(um => um.CreateAsync(It.Is<User>(u => u.Email == "new@mail.com")))
            .ReturnsAsync(IdentityResult.Failed(new IdentityError { Description = "Creation failed" }));

        var result = await sut.HandleCallbackAsync(authResult);

        result.Success.Should().BeFalse();
        result.ErrorMessage.Should().Be("Failed to create or update user");
        result.User.Should().BeNull();
    ...
    [Fact]
    public async Task HandleCallbackAsync_WhenNewUser_CreatesUserAndReturnsTokens()
    {
        var (userManager, jwtProvider, tokenStorage, _, sut) = BuildSut();
